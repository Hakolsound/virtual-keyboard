...
:: ── 2. Disable tablet-mode auto-keyboard on sign-in ──────────────────────
...
:: *** Edit TARGET_URL above before running ***

:: Build the Chrome arguments string
set "ARGS=--kiosk --disable-pinch --overscroll-history-navigation=0 --disable-features=TranslateUI,Translate --disable-session-crashed-bubble --hide-crash-restore-bubble --load-extension="%EXT_DIR%" %TARGET_URL%"

:: Write a temp PowerShell script to avoid ^ / quoting issues inside -Command
set "PS_TMP=%TEMP%\vkb_shortcut.ps1"
(
  echo $ws = New-Object -ComObject WScript.Shell
  echo $s = $ws.CreateShortcut('%SHORTCUT%')
  echo $s.TargetPath = '%CHROME%'
  echo $s.Arguments = '%ARGS%'
...
echo    Edit TARGET_URL in this script before deploying.

:: ── 4. Register TabTip kill at startup ───────────────────────────────────
echo [3/4] Registering TabTip suppressor at startup...
schtasks /create /tn "VKB-SuppressTabTip" /tr "taskkill /F /IM TabTip.exe" /sc onlogon /ru "%USERNAME%" /f >nul 2>&1
echo    Done.
